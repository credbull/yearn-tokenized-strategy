...
contract DeploySimpleTokenizedStakerTest is Test {
    function test__DeploySimpleTokenizedStaker__Deploy() public {
        ...
        assertEq(
            staker.emergencyAdmin(),
            yearnAuth.emergencyAdmin,
            "emergencyAdmin not set"
        );
        assertEq(
            staker.pendingManagement(),
            yearnAuth.management,
            "pending management not set"
        );
        assertEq(staker.keeper(), yearnAuth.keeper, "keeper not set");
        assertEq(
            staker.performanceFeeRecipient(),
            yearnAuth.perfFeeRecipient,
            "perfFeeRecipient not set"
        );
